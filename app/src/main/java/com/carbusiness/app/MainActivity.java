...
public class MainActivity extends FragmentActivity {
  private WebView webView; private FrameLayout root; private TextView privacyCover; private byte[] pendingBytes; private ValueCallback<Uri[]> fileChooserCallback;
  ...
  @Override protected void onCreate(Bundle b){
    ...
    root=new FrameLayout(this); webView=new WebView(this); root.addView(webView,new FrameLayout.LayoutParams(FrameLayout.LayoutParams.MATCH_PARENT,FrameLayout.LayoutParams.MATCH_PARENT)); privacyCover=new TextView(this); privacyCover.setText("MY WHEELS\n\nLocked"); privacyCover.setTextSize(22); privacyCover.setTextColor(Color.rgb(23,33,43)); privacyCover.setGravity(Gravity.CENTER); privacyCover.setBackgroundColor(Color.WHITE); root.addView(privacyCover,new FrameLayout.LayoutParams(FrameLayout.LayoutParams.MATCH_PARENT,FrameLayout.LayoutParams.MATCH_PARENT)); setContentView(root);WebSettings s=webView.getSettings();s.setJavaScriptEnabled(true);s.setDomStorageEnabled(true);s.setDatabaseEnabled(true);s.setAllowFileAccess(true);s.setAllowContentAccess(true);
    ...
    if(isLockEnabled()) showUnlock(); else {unlocked=true;setPrivateCover(false);}
  }
  ...
  private void setPrivateCover(boolean locked){runOnUiThread(()->{if(privacyCover!=null)privacyCover.setVisibility(locked?android.view.View.VISIBLE:android.view.View.GONE);});}
  private void showUnlock(){
    if(unlocked){setPrivateCover(false);return;} setPrivateCover(true);
  ...
  private void doBiometric(){
    BiometricPrompt p=new BiometricPrompt(this,ContextCompat.getMainExecutor(this),new BiometricPrompt.AuthenticationCallback(){@Override public void onAuthenticationSucceeded(BiometricPrompt.AuthenticationResult r){unlocked=true;setPrivateCover(false);}@Override public void onAuthenticationError(int c,CharSequence e){showPinDialog();}});
  ...
  private void showPinDialog(){
    ...
    new AlertDialog.Builder(this).setTitle("My Wheels Locked").setMessage("Enter PIN. Use Recovery if you forgot it.").setView(e).setCancelable(false).setPositiveButton("Unlock",(d,w)->{if(sha(e.getText().toString()).equals(prefs.getString("pin_hash",""))){unlocked=true;setPrivateCover(false);}else{new AlertDialog.Builder(this).setMessage("Incorrect PIN").setPositiveButton("Try again",(a,q)->showPinDialog()).show();}}).setNegativeButton("Recovery",(d,w)->showRecoveryDialog()).show();
  }
  private void showRecoveryDialog(){
    EditText e=new EditText(this);e.setHint("Recovery code");
    new AlertDialog.Builder(this).setTitle("Recover App Access").setMessage("Enter your recovery code. Your business data will not be deleted.").setView(e).setCancelable(false).setPositiveButton("Verify",(d,w)->{if(sha(e.getText().toString().trim().toUpperCase(Locale.ROOT)).equals(prefs.getString("recovery_hash",""))){prefs.edit().remove("pin_hash").apply();unlocked=true;setPrivateCover(false);js("toast('PIN reset. Set a new PIN in Settings.')");}else showPinDialog();}).setNegativeButton("Back",(d,w)->showPinDialog()).show();
  }
  ...
  @Override protected void onPause(){backgroundAt=System.currentTimeMillis();if(isLockEnabled())setPrivateCover(true);super.onPause();}
  public class AndroidBridge{
    ...
    @JavascriptInterface public void printReport(String title,String html){runOnUiThread(()->{try{WebView pv=new WebView(MainActivity.this);pv.getSettings().setJavaScriptEnabled(false);pv.setWebViewClient(new WebViewClient(){@Override public void onPageFinished(WebView view,String url){PrintManager pm=(PrintManager)getSystemService(Context.PRINT_SERVICE);pm.print(title,view.createPrintDocumentAdapter(title),null);}});pv.loadDataWithBaseURL(null,html,"text/html","UTF-8",null);}catch(Exception e){js("toast('Could not open print dialog')");}});}
